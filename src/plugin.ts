...
export class StripeTerminalPlugin {
  ...
  private _onPaymentStatusChange: (d:string) => void = () => {
    // reset the sdk type
    this.selectedSdkType = 'native'

    return Promise.reject('You must initialize StripeTerminalPlugin first.')
  }
  private _onConnectionStatusChange: (d:string) => void = () => {
    // reset the sdk type
    this.selectedSdkType = 'native'
  ...
  private async init() {
    ...
    this.listeners['unexpectedReaderDisconnectListenerNative'] =
      ...
      await StripeTerminal.addListener('didChangePaymentStatus', (d:string) => {
          this._onPaymentStatusChange(d);
      })

      this.listeners['changeConnectionStatusListenerNative'] =
      await StripeTerminal.addListener('didChangeConnectionStatus', (d:string) => {
