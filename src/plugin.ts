...
export class StripeTerminalPlugin {
  ...
  private _onPaymentStatusChange: (d:any) => any = () => {
    this.selectedSdkType = 'native'
    return Promise.reject('You must initialize StripeTerminalPlugin first.')
  }
  private _onConnectionStatusChange: (d:any) => any = () => {
    this.selectedSdkType = 'native'
    return Promise.reject('You must initialize StripeTerminalPlugin first.')
  }
  private _onReportReaderSoftwareUpdateProgress: (d:any) => any = () => {
    this.selectedSdkType = 'native'
    return Promise.reject('You must initialize StripeTerminalPlugin first.')
  }
  private _onFinishInstallingUpdate: (d:any) => any = () => {
    this.selectedSdkType = 'native'
    return Promise.reject('You must initialize StripeTerminalPlugin first.')
  }
  private _onReportAvailableUpdate: (d:any) => any = () => {
    this.selectedSdkType = 'native'
    return Promise.reject('You must initialize StripeTerminalPlugin first.')
  }
  private _onStartInstallingUpdate: (d:any) => any = () => {
    this.selectedSdkType = 'native'
    return Promise.reject('You must initialize StripeTerminalPlugin first.')
  }
  ...
  public paymentStatus(): Observable<PaymentStatus> {
    ...
    return new Observable(subscriber => {
      ...
      // then listen for changes
      StripeTerminal.addListener('didChangePaymentStatus', (data: any) => {
        // only send an event if we are currently on this sdk type
        if (this.activeSdkType === 'native') {
          hasSentEvent = true
          subscriber.next(data?.status)
      ...
      // then listen for js changes
      this.stripeTerminalWeb
        ?.addListener('didChangePaymentStatus', (data: any) => {
          // only send an event if we are currently on this sdk type
          if (this.activeSdkType === 'js') {
            hasSentEvent = true
            subscriber.next(data?.status)
  ...
  public async cleanupTerminal(): Promise<void> {
    this.ensureInitialized()
